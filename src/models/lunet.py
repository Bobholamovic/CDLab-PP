...
# Transferred from https://github.com/mpapadomanolaki/multi-task-L-UNet/blob/main/code/network.py
    ...
    def forward(self, x):
        ...
        for t in range(n):
            ...
            if h_state is None:
                ...
                h_state = paddle.zeros((b,self.hidden_size,h,w))
                c_state = paddle.zeros((b,self.hidden_size,h,w))
            h_state, c_state = self.rnn_cell(input_t, h_state, c_state)

        return h_state, paddle.stack(out_list, axis=1)


class LUNet(nn.Layer):
    ...
    def decode(self, x1, x2, x3, x4, x5):
        d5 = self.up5(x5)
        d5 = paddle.concat([d5,x4], 1)
        d5 = self.conv5d(d5)

        d4 = self.up4(d5)
        d4 = paddle.concat([d4,x3], 1)
        d4 = self.conv4d(d4)

        d3 = self.up3(d4)
        d3 = paddle.concat([d3,x2], 1)
        d3 = self.conv3d(d3)

        d2 = self.up2(d3)
        d2 = paddle.concat([d2,x1], 1)
        d2 = self.conv2d(d2)
